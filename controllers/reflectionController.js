...
class reflectionController {
  ...
  static async updateReflectionsById(req, res) {
    // res.send("ok");
    try {
      const { succes, low_point, take_away } = req.body;
      const { id } = req.params;
      await db.query(``, (err, result, field) => {
        const { id: UserId } = req.UserData;
        db.query(`select * from "Reflections" where id=$1 and "UserId"=;`, [id, UserId], (err, result, field) => {
